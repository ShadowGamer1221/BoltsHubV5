...
local name="Bolts Ware v11 - Fully Undetected - "..identifyexecutor()..""
...
CombatTabMainSection:Button("KILL ALL (OP)",function()
...
local WeaponMechanics = require(game:GetService("ReplicatedFirst").Services.Weapons.WeaponMechanics)
...
function ClosestPlayer()
    ...
    for i,v in next, Players:GetPlayers() do
        if v ~= Player then
            --if v.Character and v.Character:FindFirstChild("Head") and v.Character.Humanoid.Health > 0 and v.TeamColor ~= Player.TeamColor then --- creating the checks
            if v.Character and v.Character:FindFirstChild("Head") and v.Character:FindFirstChild("Gun") and v.TeamColor ~= Player.TeamColor then --- creating the checks
...
function DoStuff()
    for i,v in next, game.Players:GetPlayers() do
        if v.TeamColor ~= Player.TeamColor and v.Character and v.Character:FindFirstChild("Gun") then
...
repeat task.wait() DoStuff() until ReplicatedStorage.wkspc.Status.RoundOver.Value == true
end)
...
Title.Text = "Skid Hub V6 Aimlock"
...
CombatTabGunmodsSection:Button("2x your fireate : ON",function()
...
CombatTabGunmodsSection:Button("2x your fireate : OFF",function()
...
CombatTabGunmodsSection:Button("Automatic Weapon : ON",function()
...
CombatTabGunmodsSection:Button("Automatic Weapon : OFF",function()
...
CombatTabGunmodsSection:Button("INF AMMO : ON",function()
...
CombatTabGunmodsSection:Button("INF AMMO : OFF",function()
...
local PlayerTabAntiAimSection=PlayerTab:Section("Shit")
...
VisualsTabESPSection:Toggle("Box ESP",function(state)
    Config.Visuals.BoxEsp = state
end)

VisualsTabESPSection:Toggle("Line ESP",function(state)
    Config.Visuals.TracerEsp = state
end)

VisualsTabESPSection:Toggle("Name ESP",function(state)
    Config.Visuals.NameEsp = state
end)

VisualsTabESPSection:Toggle("Skeloten ESP",function(state)
...
MiscTabMainSection:Button("Crash Server (fuck you xonae)",function()
    while true do wait(2)
for i = 1, 500 do
    game.ReplicatedStorage.Network.LoadCharacter:FireServer()
    game.ReplicatedStorage.Network.SpawnMe:FireServer()

end
end
end)

MiscTabMainSection:Toggle("Chat Spam (xonae is a bitch)",function(state)
...
spawn(function()
    while true do
        ...
        local args = {
    [1] = Client.Values.ChatMsg
}

game:GetService("ReplicatedStorage").Network.Chat:FireServer(unpack(args))
...
MiscTabMainSection:Textbox("Custom Chat Message","Bolts Ware V11 On Top",function(x)
...
W:Button("xonae anti cheat ew",function()
    print("Button Pressed")
end)
...
getgenv().DiscordWebhook = "https://discord.com/api/webhooks/1031250979494047894/6Jz3Ks8hUoiHklITWAenoJGCf8rrLWHMp73WQEzDvPjtn7Al1KaV5-EaJ9TdXpu2Uy4G" -- Discord Webhook Here
